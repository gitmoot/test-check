...
func runProve(args []string, stdout, stderr io.Writer) int {
	...
	test := fs.String("test", "", "test command, run with sh -c at the repository root; keep it narrow")
	...
	report, err := prove.Run(ctx, prove.Options{Dir: *dir, Base: *base, Test: *test, Timeout: *timeout})
	if err != nil {
		fmt.Fprintf(stderr, "test-check prove: %v\n", err)
		return exitSource
	}
